import { commercialPage, projects } from "@/data/site";

const commercialProjects = projects.filter((project) => project.category === "commercial");

/**
 * Mirrors the Figma "Wrapper" layout (title + CTA on one side, intro
 * paragraph + stacked title/description/image blocks on the other) —
 * one block per existing project in the "commercial" category.
 */
export function CommercialListings() {
  const { listings } = commercialPage;
